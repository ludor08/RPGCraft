...
public class WeakenZombieKingIdleState extends EntityState
{
    ...
    int TIME_BETWEEN_ATTACKS = 60;

    public WeakenZombieKingIdleState()
    {
        super("weaken_zombie_king_idle_state");
    }

    @Override
    public EntityState OnTick(Entity thisEntity, int tick)
    {
        System.out.println("1 wook");

        ScanForShieldCatalysts(thisEntity);

        List<EntityState> attacks = List.of(EntityStates.WEAKEN_ZOMBIE_KING_ENHANCE_ZOMBIES.GetEntityState(), EntityStates.WEAKEN_ZOMBIE_KING_SUMMON_ZOMBIES.GetEntityState(), EntityStates.WEAKEN_ZOMBIE_KING_SHOOT_AT_PLAYER.GetEntityState());
        ...
        {
            RPGutils.AddToNamespacedKey(thisEntity, attackCooldown, TIME_BETWEEN_ATTACKS, -1);
        ...
        if (thisEntity instanceof LivingEntity living)
        {
            ...
            else
            {
                ...
                if ((living.getHealth() / living.getAttribute(Attribute.MAX_HEALTH).getValue()) <= currentDamageMilestone)
                {
                    ...
                    currentDamageMilestone = RPGutils.AddToNamespacedKey(living, damageMilestone, 1f, -0.25f);
                    e.setCancelled(true);

                    SelectNewShieldCatalysts(thisEntity);
                }

                if (currentDamageMilestone < 0.5)
                {
    ...
    public int GetTimeBetweenAttacks()
    {
        return TIME_BETWEEN_ATTACKS;
    }
}
